...
class MetricsTest(unittest.TestCase):
    def test_loads_regular_json_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "gar-stream-tx.json").write_text(json.dumps({"frames": {"sent": 3}}), encoding="utf-8")
            self.assertEqual({"frames": {"sent": 3}}, load_metrics(root, "gar-stream-tx"))
    ...
    def test_rejects_missing_symlink_large_and_non_object_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(MetricsError, "does not exist"):
                load_metrics(root, "gar-stream-tx")
            target = root / "target.json"
            target.write_text("{}", encoding="utf-8")
            (root / "gar-stream-tx.json").symlink_to(target)
            with self.assertRaisesRegex(MetricsError, "non-symlink"):
                load_metrics(root, "gar-stream-tx")
            (root / "gar-stream-tx.json").unlink()
            (root / "gar-stream-tx.json").write_bytes(b"x" * (MAX_METRICS_BYTES + 1))
            with self.assertRaisesRegex(MetricsError, "size limit"):
                load_metrics(root, "gar-stream-tx")
            (root / "gar-stream-tx.json").write_text("[]", encoding="utf-8")
            with self.assertRaisesRegex(MetricsError, "root must be an object"):
                load_metrics(root, "gar-stream-tx")
            (root / "gar-stream-tx.json").write_text('{"value":NaN}', encoding="utf-8")
