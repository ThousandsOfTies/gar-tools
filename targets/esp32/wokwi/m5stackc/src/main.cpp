...
bool hasRequiredConfig(bool logMissing = false) {
  if (strlen(kSsid) == 0) {
    if (logMissing) {
      mark("missing wifi ssid");
    }
    return false;
  }
  if (strlen(kToken) == 0) {
...
void setup() {
  ...
  digitalWrite(4, HIGH);
#if defined(VIBE_WOKWI)
  pinMode(kButtonAGpio, INPUT_PULLUP);
  pinMode(kButtonBGpio, INPUT_PULLUP);
  pinMode(kButtonPwrGpio, INPUT_PULLUP);
#else
